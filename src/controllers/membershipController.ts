...
export const getSingleMembership = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(200).json(currentMembership);
  } catch (error) {
...
export const manageMembership = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!authUser) {
      logger.warn('No authenticated user found when trying to register membership.');
      return res.status(401).json({ error: 'Unauthorized' });
    }

    const { membership_class, mappi_allowance, duration } = req.body;
    ...
    logger.info(`Registered membership for user ${authUser.pi_uid}`);
    return res.status(200).json({ membership: updatedMembership });
  } catch (error) {
    logger.error(`Failed to register membership for userID ${authUser?.pi_uid}:`, error);
    return res.status(500).json({
      message: 'An error occurred while registering membership; please try again later',
    });
  }
};
...
export const getAllReviews = async (page: number, limit: number) => {
  ...
  const user = mostReviewedUser.length > 0 
    ? await User.findOne({ pi_uid: mostReviewedUser[0]._id }) 
    : null;
  ...
  const currentMonthReviewPercentage =
    totalReviews > 0 ? ((currentMonthReviews / totalReviews) * 100).toFixed(2) : "0.00";
